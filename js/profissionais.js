document.addEventListener('DOMContentLoaded', () => {
  ...
  function atualizarResumo() {
    ...
    document.getElementById('atendimentosHoje').textContent = profissionais.reduce((total, p) => total + p.atendimentos, 0);
  }
  ...
  function renderizar() {
    ...
    const filtrados = profissionais.filter(p => {
      ...
      const combinaStatus = status === 'todos' || (status === 'ativo' && p.ativo) || (status === 'inativo' && !p.ativo);
    ...
    grid.innerHTML = filtrados.map(p => `
      <article class="profissional-card">
        <div class="profissional-header">
          <div class="profissional-identity">
            <div class="profissional-avatar">${iniciais(p.nome)}</div>
            <div>
              <h3>${p.nome}</h3>
              <p class="especialidade">${p.especialidade}</p>
            </div>
          </div>
          <span class="profissional-status ${p.ativo ? 'ativo' : 'inativo'}">${p.ativo ? 'Ativo' : 'Inativo'}</span>
        </div>

        <div class="profissional-contact">
          <span>${p.telefone || 'Telefone não informado'}</span>
          <span>${p.email || 'E-mail não informado'}</span>
        </div>

        <div class="profissional-services">
          ${p.servicos.length ? p.servicos.map(servico => `<span class="profissional-service-tag">${servico}</span>`).join('') : '<span class="profissional-service-tag">Sem serviços vinculados</span>'}
        </div>

        <div class="profissional-meta">
          <div><span>Expediente</span><strong>${p.inicio} — ${p.fim}</strong></div>
          <div><span>Atendimentos hoje</span><strong>${p.atendimentos}</strong></div>
        </div>

        <div class="profissional-actions">
          <button class="btn btn-outline-custom btn-sm" data-action="editar" data-id="${p.id}">Editar</button>
          <button class="btn btn-outline-custom btn-sm" data-action="status" data-id="${p.id}">${p.ativo ? 'Inativar' : 'Ativar'}</button>
          <button class="btn btn-outline-danger btn-sm" data-action="excluir" data-id="${p.id}">Excluir</button>
        </div>
      </article>
    `).join('');
  ...
  function editarProfissional(id) {
    ...
    campoInicio.value = profissional.inicio;
    campoFim.value = profissional.fim;
  ...
  grid.addEventListener('click', event => {
    ...
    if (button.dataset.action === 'status') {
      profissionais[index].ativo = !profissionais[index].ativo;
      renderizar();
    }
    if (button.dataset.action === 'excluir') {
      const confirmou = window.confirm(`Excluir ${profissionais[index].nome}?`);
      if (!confirmou) return;
  ...
  form.addEventListener('submit', event => {
    ...
    const dados = {
      ...
      inicio: campoInicio.value || '08:00',
      fim: campoFim.value || '18:00',
    ...
    if (id) {
      ...
      profissionais.push({ id: Date.now(), ...dados, atendimentos: 0 });
    }
