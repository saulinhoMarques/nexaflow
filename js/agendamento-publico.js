document.addEventListener('DOMContentLoaded', () => {
  ...
  const fallbackCompanies = {
    'barbearia-imperial': {
      ...
      servicos: [{ nome: 'Corte masculino', preco: 35, duracao: 30 }, { nome: 'Barba', preco: 25, duracao: 20 }, { nome: 'Corte + Barba', preco: 55, duracao: 50 }, { nome: 'Sobrancelha', preco: 15, duracao: 15 }],
      profissionais: [{ nome: 'Carlos Mendes', especialidade: 'Barbeiro', servicos: ['Corte masculino', 'Barba'] }, { nome: 'Rafael Alves', especialidade: 'Barbeiro', servicos: ['Corte masculino', 'Corte + Barba'] }, { nome: 'Marina Lima', especialidade: 'Cabeleireira', servicos: ['Sobrancelha'] }],
  ...
  if (storedCompany?.slug === slug && storedCompany.empresa) empresa = storedCompany.empresa;
  ...
  function renderServices() {
    serviceOptions.innerHTML = empresa.servicos.length ? empresa.servicos.map(s => `<button class="option-card ${state.servico?.nome === s.nome ? 'selected' : ''}" type="button" data-service="${s.nome}"><h3>${s.nome}</h3><p>${s.descricao || 'Atendimento disponível para agendamento online.'}</p><div class="option-meta"><span>${s.duracao || 30} min</span><span>${moeda(s.preco)}</span></div></button>`).join('') : '<div class="booking-empty">Nenhum serviço disponível.</div>';
  }
  ...
  function renderProfessionals() {
    ...
    professionalOptions.innerHTML = available.length ? available.map(p => `<button class="option-card ${state.profissional?.nome === p.nome ? 'selected' : ''}" type="button" data-professional="${p.nome}"><h3>${p.nome}</h3><p>${p.especialidade || 'Profissional'}</p><div class="option-meta"><span>${(p.servicos || []).join(' · ')}</span><span>Disponível</span></div></button>`).join('') : '<div class="booking-empty">Nenhum profissional disponível para este serviço.</div>';
  }
  ...
  function renderDates() {
    ...
    dateOptions.innerHTML = dates.length ? dates.map(item => `<button class="date-option ${state.data === item.value ? 'selected' : ''}" type="button" data-date="${item.value}"><small>${new Intl.DateTimeFormat('pt-BR', { weekday: 'short' }).format(item.date)}</small>${new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: '2-digit' }).format(item.date)}</button>`).join('') : '<div class="booking-empty">A empresa não possui dias abertos configurados.</div>';
  }

  function allBookings() {
    const publicos = getPublicBookings().filter(b => b.companySlug === slug);
    const internos = getInternalBookings().filter(b => !b.companySlug || b.companySlug === slug);
  ...
  function confirmBooking() {
    ...
    const booking = {
      ...
      servico: state.servico.nome, duracao: Number(state.servico.duracao) || 30, profissional: state.profissional.nome,
      data: state.data, hora: state.hora, status: 'pendente', observacoes: customerNotes.value.trim(), origem: 'publico', createdAt: new Date().toISOString()
    };
